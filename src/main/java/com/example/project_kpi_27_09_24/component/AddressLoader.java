...
public class AddressLoader implements CommandLineRunner {
    ...
    //            });
                ...
                quarterData.forEach(quarterDto -> {
                    districtRepository.findByDistrictId(quarterDto.getDistrict_id())
                            .ifPresentOrElse(district -> {
                                // Agar district topilsa, uni saqlaymiz va quarter yaratilishini tekshiramiz
                                if (!quarterRepository.existsByQuarterId(quarterDto.getId())) {
                                    Quarter quarter = quarterRepository.save(new Quarter(quarterDto.getId(), quarterDto.getName(), district));
                                    System.out.println(quarter);
                                } else {
                                    System.out.println("Quarter already exists for quarter_id: " + quarterDto.getId());
                                }
                            }, () -> {
                                // District mavjud bo'lmasa, xato xabari chiqaramiz
